    ...
    public class SubstanceGraphSOEditor : UnityEditor.Editor
    {
        ...
        public void OnEnable()
        {
            ...
            if (_inputGroupingHelper == null)
                _inputGroupingHelper = new GraphInputsGroupingHelper(_target.Graph, serializedObject);

            if (_outputChannelsHelper == null)
                _outputChannelsHelper = new GraphOutputAlphaChannelsHelper(_target.Graph);
        ...
        public override void OnInspectorGUI()
        {
            if (_materialPreviewEditor == null)
            {
                var material = _target.Graph.OutputMaterial;
            ...
            SerializedProperty rootGraph = serializedObject.FindProperty("Graph");
        ...
        {
            return (_materialPreviewEditor) ? true : false;
        }
        ...
        private bool DrawGraph(SerializedProperty graph)
        {
            ...
            DrawPresentExport(graph, _target.Graph, content);
        ...
        {
            if (_target.Graph.HasThumbnail)
                return _target.Graph.GetThumbnailTexture();
        ...
        public void OnSceneDrag(SceneView sceneView, int index)
        {
            ...
            var graph = _target.Graph;

            if (graph.OutputMaterial != null)
            {
                if (go && go.GetComponent<Renderer>())
                {
                    HandleRenderer(go.GetComponent<Renderer>(), materialIndex, _target.Graph.OutputMaterial, evt.type, evt.alt);

                    if (graph.IsRuntimeOnly)
                    {
                        var runtimeComponent = go.GetComponent<Runtime.SubstanceRuntimeGraph>();

                        if (runtimeComponent == null)
                            runtimeComponent = go.AddComponent<Runtime.SubstanceRuntimeGraph>();
        ...
        private void SaveTGAFiles()
        {
            ...
            foreach (var output in graph.Output)
            {
                ...
                {
                    assetPath = graph.GetAssociatedAssetPath(output.Description.Identifier, "tga");
                    output.OutputTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(assetPath);
                    targetTexture = output.OutputTexture;
                }
