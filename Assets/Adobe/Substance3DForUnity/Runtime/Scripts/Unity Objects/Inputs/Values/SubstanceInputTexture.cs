...
namespace Adobe.Substance.Input
{
    ...
    public class SubstanceInputTexture : ISubstanceInput
    {
        ...
        public void UpdateNativeHandle(SubstanceNativeHandler handler)
        {
            ...
            if (!_Data.isReadable)
            {
                Debug.LogWarning($"Input textures must be set as readable. Texture assigned to {_Description.Identifier} will have no effect.");
                return;
            }

            var pixels = _Data.GetPixels32();
            handler.SetInputTexture2D(pixels, _Data.width, _Data.height, _Index, _GraphID);
        }
